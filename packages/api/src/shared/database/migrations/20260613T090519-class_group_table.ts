...
export async function up(db: Kysely<PikSlotsDatabase>): Promise<void> {
  ...
  await db.schema
    .createIndex('idx_class_group_business_id_name')
    .on('class_groups')
  ...
  await db.schema
    .createIndex('idx_class_group_business_id')
    .on('class_groups')
    .column('business_id')
    .execute();

  await db.schema
    .createIndex('idx_class_group_is_deleted')
    .on('class_groups')
    .column('is_deleted')
    .execute();
}
...
export async function down(db: Kysely<PikSlotsDatabase>): Promise<void> {
  await db.schema.dropTable('class_groups').execute();
}
