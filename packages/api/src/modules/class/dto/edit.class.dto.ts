...
import { UpdateClassInput } from '@pikslots/shared';
import {
  ArrayMaxSize,
  IsArray,
  IsBoolean,
...
export class EditClassDto implements Omit<UpdateClassInput, 'id'> {
  @ApiProperty({
    example: 'Yoga Basics',
  ...
  @ApiProperty({
    type: [String],
    example: [],
    description: 'Class group IDs to associate this class with',
  })
  @IsArray()
  @IsUUID('7', { each: true })
